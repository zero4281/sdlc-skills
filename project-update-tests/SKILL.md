...
- You are acting SOLELY as an **orchestrator**. You must NOT perform any work yourself.
- You must NOT read files, analyze code, write code, or execute terminal commands directly in the main interface.
- **ALL work**—including initial file reads, directory inspection, analysis, fixes, and running test commands—MUST be delegated entirely to an `@general` agent.
- Your ONLY responsibility is creating tasks via `todowrite`, spawning `@general` agents to perform the work, and reviewing their returned summaries.
...
**Constraint:** You must execute these tasks sequentially. Do not batch them. Wait for the summary of each task before creating the next. You must NOT read the files to perform this analysis yourself—delegate everything to `@general`.
...
- **Unresolvable Blockers & Bug Filing:** If a test fails due to a bug or limitation in core source code (which cannot be modified per constraints):
  ...
  4. Mark the task as blocked due to an open core bug report.

**WAIT:** Do not begin Phase 3 until all fix tasks are marked complete or explicitly reported as blocked with a filed bug report.
...
2. **Iterative Repair Loop:**
   ...
   - For each failure, use `todowrite` to create a separate task assigned to an `@general` agent. 
   ...
   - Execute these fix tasks one at a time.

3. **Final Suite Validation:**
   Once all fix tasks are complete, assign a final task to an `@general` agent to **re-run the entire test suite execution command** extracted from `./Testing Strategy.md`. 
