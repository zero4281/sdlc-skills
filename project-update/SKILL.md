...
> Read `./Plan.md` to establish the target baseline. Treat Plan.md as the verified source of current project goals, but **explicitly ignore Section 3 (Testing & Verification Status)** as testing is handled by a separate pipeline.
...
> 2. Implemented features currently in the source code that violate the plan constraints or are explicitly omitted by the current version of the plan (features to remove).
...
> - An explicit section detailing "Implemented but Non-Required: Features to Remove" (specifying file targets, line ranges, and rationale).
> - A structured compliance table mapping out the codebase status against sections 1, 2, and 4-7 of the plan.
